...
if(file.exists(projectDir)) {
    ...
    powerConsumptionDT$DateTime <- strptime(paste(powerConsumptionDT$Date,powerConsumptionDT$Time,sep = " "),format="%d/%m/%Y %H:%M:%S")
    
    #Create Plot 1 PNG
    png(file="plot3.png",width = 480, height = 480, units = "px")
    with(powerConsumptionDT,plot(DateTime,Sub_metering_1,type="l",ylab="Energy sub metering",col="red"))
    lines(powerConsumptionDT$DateTime,powerConsumptionDT$Sub_metering_2,col="orange")
    lines(powerConsumptionDT$DateTime,powerConsumptionDT$Sub_metering_3,col="blue")
    dev.off()
